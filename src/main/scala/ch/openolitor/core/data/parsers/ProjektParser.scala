...
object ProjektParser extends EntityParser {
  ...
  def parse(implicit loggingAdapter: LoggingAdapter) = {
    parseEntity[Projekt, ProjektId]("id", Seq("bezeichnung", "strasse", "haus_nummer", "adress_zusatz", "plz", "ort",
      ...
      "generierte_mails_senden", "einsatz_einheit", "einsatz_absage_vorlauf_tage", "einsatz_show_liste_kunde", "send_email_to_bcc", "default_payment_type") ++ modifyColumns) { id => indexes =>
      row =>
        ...
        val Seq(indexBezeichnung, indexStrasse, indexHausNummer, indexAdressZusatz, indexPlz, indexOrt, indexPreiseSichtbar,
          ...
          indexGenerierteMailsSenden, indexEinsatzEinheit, indexEinsatzAbsageVorlaufTage, indexEinsatzShowListeKunde, indexSendEmailToBcc, indexDefaultPaymentType) = indexes take (23)
        ...
        Projekt(
          ...
          defaultPaymentType = row.value[Option[String]](indexDefaultPaymentType),
          //modification flags
