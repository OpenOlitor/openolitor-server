...
object ProjektParser extends EntityParser {
  ...
  def parse(implicit loggingAdapter: LoggingAdapter) = {
    parseEntity[Projekt, ProjektId]("id", Seq("bezeichnung", "strasse", "haus_nummer", "adress_zusatz", "plz", "ort",
      ...
      "generierte_mails_senden", "einsatz_einheit", "einsatz_absage_vorlauf_tage", "einsatz_show_liste_kunde", "send_email_to_bcc", "messageForMembers") ++ modifyColumns) { id => indexes =>
      row =>
        ...
        val Seq(indexMessageForMembers) = indexes takeRight (1)
        ...
        Projekt(
          ...
          messageForMembers = row.value[Option[String]](indexMessageForMembers),
          //modification flags
