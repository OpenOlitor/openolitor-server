...
object RecalculateAnzahlAbosCounter {
  val scripts = new Script with LazyLogging with StammdatenDBMappings with DefaultDBScripts {
    def execute(sysConfig: SystemConfig)(implicit session: DBSession): Try[Boolean] = {
      sql"""update ${depotMapping.table} u SET u.anzahl_abonnenten=0""".execute.apply()

      sql"""update ${depotMapping.table} u
        INNER JOIN (SELECT a.depot_id, count(a.id) counter FROM ${depotlieferungAboMapping.table} a
        group by a.depot_id) a ON u.id=a.depot_id
        SET u.anzahl_abonnenten=a.counter""".execute.apply()

      sql"""update ${tourMapping.table} u SET u.anzahl_abonnenten=0""".execute.apply()

      sql"""update ${tourMapping.table} u
        INNER JOIN (SELECT a.tour_id, count(a.id) counter FROM ${heimlieferungAboMapping.table} a
        group by a.tour_id) a ON u.id=a.tour_id
        SET u.anzahl_abonnenten=a.counter""".execute.apply()

      sql"""update ${depotlieferungMapping.table} u SET u.anzahl_abos=0""".execute.apply()
      ...
      sql"""update ${depotlieferungMapping.table} u
        ...
        group by a.vertriebsart_id) a ON u.id=a.vertriebsart_id
        SET u.anzahl_abos=a.counter""".execute.apply()

      sql"""update ${heimlieferungMapping.table} u SET u.anzahl_abos=0""".execute.apply()
      ...
      sql"""update ${postlieferungMapping.table} u SET u.anzahl_abos=0""".execute.apply()
      ...
      sql"""update ${abotypMapping.table} u SET u.anzahl_abonnenten=0""".execute.apply()
      ...
      sql"""update ${abotypMapping.table} u
        ...
        group by d.abotyp_id) d ON u.id=d.abotyp_id
        LEFT OUTER JOIN (SELECT t.abotyp_id, count(t.id) counter FROM ${heimlieferungAboMapping.table} t
        group by t.abotyp_id) t ON u.id=t.abotyp_id
        LEFT OUTER JOIN (SELECT p.abotyp_id, count(p.id) counter FROM ${postlieferungAboMapping.table} p
        group by p.abotyp_id) p ON u.id=p.abotyp_id
        SET u.anzahl_abonnenten=COALESCE(d.counter, 0) + COALESCE(t.counter, 0) + COALESCE(p.counter, 0)""".execute.apply()

      sql"""update ${kundeMapping.table} u SET u.anzahl_abos=0""".execute.apply()
      ...
      sql"""update ${kundeMapping.table} u
        ...
        group by d.kunde_id) d ON u.id=d.kunde_id
        LEFT OUTER JOIN (SELECT t.kunde_id, count(t.id) counter FROM ${heimlieferungAboMapping.table} t
        group by t.kunde_id) t ON u.id=t.kunde_id
        LEFT OUTER JOIN (SELECT p.kunde_id, count(p.id) counter FROM ${postlieferungAboMapping.table} p
        group by p.kunde_id) p ON u.id=p.kunde_id
        SET u.anzahl_abos=COALESCE(d.counter, 0) + COALESCE(t.counter, 0) + COALESCE(p.counter, 0)""".execute.apply()
