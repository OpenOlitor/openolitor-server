...
object OO900_Guthaben {
  val DropColsFromZusatzabos = new Script with LazyLogging with StammdatenDBMappings {
...
""".execute.apply()
  ...
  val DropColsFromZusatzaboTyp = new Script with LazyLogging with StammdatenDBMappings {
    def execute(sysConfig: SystemConfig)(implicit session: DBSession): Try[Boolean] = {
      sql"""
...
""".execute.apply()
  ...
  val scripts = Seq(DropColsFromZusatzabos, DropColsFromZusatzaboTyp)
}
