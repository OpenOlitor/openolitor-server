...
import ch.openolitor.core.models.{ BaseId, BaseStringId, PersonId }
...
trait DBMappings extends BaseParameter
  ...
  // scalikejdbc unfortunately uses scala.collection.Seq instead of the migrated type scala.collection.immutable.Seq
  implicit def toImmutableSeq[T](seq: scala.collection.Seq[T]): scala.collection.immutable.Seq[T] =
    seq.toSeq
  ...
  def seqSqlBinder[V](f: String => V, g: V => String)(implicit classTag: ClassTag[V]): Binders[Seq[V]] = Binders.string.xmap({
    Option(_) match {
  ...
  def setSqlBinder[V](f: String => V, g: V => String)(implicit classTag: ClassTag[V]): Binders[Set[V]] = Binders.string.xmap({
    Option(_) match {
  ...
  def seqBaseIdBinders[T <: BaseId](f: Long => T)(implicit classTag: ClassTag[T]): Binders[Seq[T]] = seqSqlBinder[T](l => f(l.toLong), _.id.toString)
  def setBaseIdBinders[T <: BaseId](f: Long => T)(implicit classTag: ClassTag[T]): Binders[Set[T]] = setSqlBinder[T](l => f(l.toLong), _.id.toString)
  def setBaseStringIdBinders[T <: BaseStringId](f: String => T)(implicit classTag: ClassTag[T]): Binders[Set[T]] = setSqlBinder[T](l => f(l), _.id)
  ...
  implicit val localDateBinder = JodaBinders.jodaLocalDate
  implicit val optionLocalDateBinder = Binders.option[LocalDate]
  implicit val datetimeBinder = JodaBinders.jodaDateTime
