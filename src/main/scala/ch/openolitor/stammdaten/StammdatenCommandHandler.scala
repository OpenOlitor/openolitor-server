...
trait StammdatenCommandHandler extends CommandHandler with StammdatenDBMappings with ConnectionPoolContextAware
  ...
  override val handle: PartialFunction[UserCommand, IdFactory => EventTransactionMetadata => Try[Seq[ResultingEvent]]] = {
    ...
    case SendEmailToKundenCommand(_, subject, body, ids) => _ => _ =>
      DB readOnly { implicit session =>
    ...
    case SendEmailToPersonenCommand(_, subject, body, ids) => _ => _ =>
      DB readOnly { implicit session =>
    ...
    case SendEmailToAbotypSubscribersCommand(_, subject, body, ids) => _ => _ =>
      DB readOnly { implicit session =>
    ...
    case SendEmailToZusatzabotypSubscribersCommand(_, subject, body, ids) => _ => _ =>
      DB readOnly { implicit session =>
        if (checkTemplateAbotyp(body, subject, ids)) {
          val events = ids flatMap { abotypId =>
    ...
    case SendEmailToTourSubscribersCommand(_, subject, body, ids) => _ => _ =>
      DB readOnly { implicit session =>
    ...
    case SendEmailToDepotSubscribersCommand(_, subject, body, ids) => _ => _ =>
      DB readOnly { implicit session =>
    ...
    case SendEmailToAbosSubscribersCommand(_, subject, body, ids) => _ => _ =>
      DB readOnly { implicit session =>
