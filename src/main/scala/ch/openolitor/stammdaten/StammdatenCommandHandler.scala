    ...
    case e @ InsertEntityCommand(personId, entity: KundeModify) => idFactory => meta =>
      if (entity.ansprechpersonen.isEmpty) {
        ...
        val kontoDaten = entity.kontoDaten match {
          case Some(kd) => KontoDatenModify(kd.iban, None, None, kd.bankName, kd.nameAccountHolder, kd.addressAccountHolder, Some(kundeId), None, kd.bic)
          case None     => KontoDatenModify(None, None, None, None, None, None, Some(kundeId), None, None)
        }
