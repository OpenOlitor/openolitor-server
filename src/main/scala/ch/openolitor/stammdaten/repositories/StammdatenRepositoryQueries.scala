...
trait StammdatenRepositoryQueries extends LazyLogging with StammdatenDBMappings with ArbeitseinsatzDBMappings {
  ...
  private def getDepotAuslieferungQuery[A](auslieferungId: AuslieferungId)(f: (DepotAuslieferung, Depot, Seq[Korb], Seq[Lieferposition], Seq[DepotlieferungAbo], Seq[Abotyp], Seq[Kunde], Seq[PersonDetail], Seq[ZusatzAbo]) => A) = {
    withSQL {
      select
        ...
        .leftJoin(kundeMapping as kunde).on(depotlieferungAbo.kundeId, kunde.id)
        .leftJoin(personMapping as person).on(kunde.id, person.kundeId)
        .leftJoin(zusatzAboMapping as zusatzAbo).on(sqls"${depotlieferungAbo.id} = ${zusatzAbo.hauptAboId} and ${depotAuslieferung.datum} >= ${zusatzAbo.start} and((${depotAuslieferung.datum} <= ${zusatzAbo.ende}) or (${zusatzAbo.ende} is null))")
      ...
      .map((auslieferung, depots, koerbe, lieferpositionen, abos, abotypen, kunden, personen, zusatzAbos) => {
        val abosWithOnlyActiveZusatzabos = abos map { mainabo =>
          val zusatzAbosForMainabo = zusatzAbos.filter(_.hauptAboId == mainabo.id)
          val zusatzAboIds = zusatzAbosForMainabo.map(_.id).toSet
          val zusatzAbotypNames = zusatzAbosForMainabo.map(_.abotypName).toSeq
          copyTo[DepotlieferungAbo, DepotlieferungAbo](mainabo, "zusatzAboIds" -> zusatzAboIds, "zusatzAbotypNames" -> zusatzAbotypNames)
        }
        val personenDetails = personen map (p => copyTo[Person, PersonDetail](p))
        f(auslieferung, depots.head, koerbe, lieferpositionen, abosWithOnlyActiveZusatzabos, abotypen, kunden, personenDetails.distinct, zusatzAbos)
      }).single
  }
  ...
  private def getTourAuslieferungQuery[A](auslieferungId: AuslieferungId)(f: (TourAuslieferung, Tour, Seq[Korb], Seq[Lieferposition], Seq[HeimlieferungAbo], Seq[Abotyp], Seq[Kunde], Seq[PersonDetail], Seq[ZusatzAbo]) => A) = {
    withSQL {
      select
        ...
        .leftJoin(personMapping as person).on(kunde.id, person.kundeId)
        .leftJoin(zusatzAboMapping as zusatzAbo).on(sqls"${heimlieferungAbo.id} = ${zusatzAbo.hauptAboId} and ${tourAuslieferung.datum} >= ${zusatzAbo.start} and((${tourAuslieferung.datum} <= ${zusatzAbo.ende}) or (${zusatzAbo.ende} is null))")
      ...
      .map((auslieferung, tour, koerbe, lieferposition, abos, abotypen, kunden, personen, zusatzAbos) => {
        val abosWithOnlyActiveZusatzabos = abos map { mainabo =>
          ...
          val zusatzAboIds = zusatzAbosForMainabo.map(_.id).toSet
          val zusatzAbotypNames = zusatzAbosForMainabo.map(_.abotypName).toSeq
          copyTo[HeimlieferungAbo, HeimlieferungAbo](mainabo, "zusatzAboIds" -> zusatzAboIds, "zusatzAbotypNames" -> zusatzAbotypNames)
        }
        val personenDetails = personen map (p => copyTo[Person, PersonDetail](p))
        f(auslieferung, tour.head, koerbe, lieferposition, abosWithOnlyActiveZusatzabos, abotypen, kunden, personenDetails.distinct, zusatzAbos)
      }).single
  }
  ...
  private def getPostAuslieferungQuery[A](auslieferungId: AuslieferungId)(f: (PostAuslieferung, Seq[Korb], Seq[Lieferposition], Seq[PostlieferungAbo], Seq[Abotyp], Seq[Kunde], Seq[PersonDetail], Seq[ZusatzAbo]) => A) = {
    withSQL {
      select
        ...
        .leftJoin(personMapping as person).on(kunde.id, person.kundeId)
        .leftJoin(zusatzAboMapping as zusatzAbo).on(sqls"${postlieferungAbo.id} = ${zusatzAbo.hauptAboId} and ${postAuslieferung.datum} >= ${zusatzAbo.start} and((${postAuslieferung.datum} <= ${zusatzAbo.ende}) or (${zusatzAbo.ende} is null))")
      ...
      .map((auslieferung, koerbe, lieferpositionen, abos, abotypen, kunden, personen, zusatzAbos) => {
        val abosWithOnlyActiveZusatzabos = abos map { mainabo =>
          ...
          val zusatzAboIds = zusatzAbosForMainabo.map(_.id).toSet
          val zusatzAbotypNames = zusatzAbosForMainabo.map(_.abotypName).toSeq
          copyTo[PostlieferungAbo, PostlieferungAbo](mainabo, "zusatzAboIds" -> zusatzAboIds, "zusatzAbotypNames" -> zusatzAbotypNames)
        }
        val personenDetails = personen map (p => copyTo[Person, PersonDetail](p))
        f(auslieferung, koerbe, lieferpositionen, abosWithOnlyActiveZusatzabos, abotypen, kunden, personenDetails.distinct, zusatzAbos)
      }).single
  }
  ...
  private def getKorbReports(koerbe: Seq[Korb], lieferpositionen: Seq[Lieferposition], abos: Seq[HauptAbo], abotypen: Seq[Abotyp], kunden: Seq[Kunde], personen: Seq[PersonDetail], zusatzAbos: Seq[ZusatzAbo]): Seq[KorbReport] = {
    koerbe.map { korb =>
      for {
        korbAbo <- abos.filter(_.id == korb.aboId).headOption
        abotyp <- abotypen.filter(_.id == korbAbo.abotypId).headOption
