...
class LoginRouteServiceSpec extends Specification with Mockito with NoTimeConversions {
  ...
  val personAdminInactive = Person(personId, KundeId(1), None, "Test", "Test", Some(email), None, None, None, None, 1, false, Some(pwdHashed.toCharArray), None,
    false, Some(AdministratorZugang), Set.empty, DateTime.now, PersonId(1), DateTime.now, PersonId(1))
  val projekt = Projekt(ProjektId(1), "Test", None, None, None, None, None, true, true, true, CHF, 1, 1, Map(AdministratorZugang -> true, KundenZugang -> false), Locale.GERMAN, None, None, false, false, EinsatzEinheit("Tage"), 1, false, DateTime.now, PersonId(1), DateTime.now, PersonId(1))
