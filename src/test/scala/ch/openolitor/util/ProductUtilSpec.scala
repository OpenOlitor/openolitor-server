...
class ProductUtilSpec extends Specification {
  ...
  "ProductUtil" should {
    "create map of simple case class" in {
      ...
      simple.toMap() === Map("simpleCaseClass" -> Map("name" -> "myName", "counter" -> 42))
    }
    ...
    "create map of simple case class with collection" in {
      ...
      simple.toMap() === Map("simpleCaseClass" -> Map("name" -> "myName", "counters" -> Seq(1, 2, 3)))
    }
    ...
    "create map of nested case class" in {
      ...
      simple.toMap() === Map("person" -> Map("name" -> "myName", "address" -> Map("street" -> "myStreet")))
    }
    ...
    "create map of nested case class in collection" in {
      ...
      simple.toMap() === Map("person" -> Map("name" -> "myName", "addresses" -> Seq(Map("street" -> "myStreet1"), Map("street" -> "myStreet2"))))
    }
  }
  ...
  "ProductUtil with converters" should {
    "convert dateformat correctly" in {
      ...
      simple.toMap({ case x: DateTime => format.print(x) }) === Map("simpleCaseClass" -> Map("name" -> "myName", "age" -> 42, "birthdate" -> "2001-15-02 01:30:15+0000"))
    }
  }
}