...
class MaxJsonPathFunctionsSpec extends Specification with Matchers {
  "Max of values" should {
    "calculate correct max if all values are numbers" in {
      JsonPathFunctions.Max.evaluate(Vector(JsNumber(2), JsNumber(3), JsNumber(4))) shouldEqual Some(Vector(JsNumber(4)))
    }

    "Parse numbers from strings" in {
      JsonPathFunctions.Max.evaluate(Vector(JsString("2"), JsNumber(3), JsString("4"))) shouldEqual Some(Vector(JsNumber(4)))
    }
